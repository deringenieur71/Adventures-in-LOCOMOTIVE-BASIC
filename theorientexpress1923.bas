100 MODE 2:INK 0,26:INK 1,0:BORDER 26:DEFINT a-z:ON ERROR GOTO 3170
105 DEFINT a-z
110 LOCATE 27,10:PRINT"The Orient Express, 1923"
120 LOCATE 28,13:PRINT"(c) David H. Ahl, 1986"
125 LOCATE 26,16:PRINT CHR$(24)+"Converted to CPC 2017 by SRS"+CHR$(24):LOCATE 27,23
...
160 PRINT TAB(22)"The Mysterious Arms Deal":PRINT:GOSUB 1720
...
240 GOSUB 2540
250 GOSUB 2770
260 PRINT"Press any key to call a taxi":WHILE LEN(INKEY$)=0:WEND:cls
290 FOR J=1 TO 24
300 PRINT:PRINT"February"DA(J)+13+HY"1923"
310 TN=18-INT(27*RND(1)):TB=TA(J)+TN:T=TB:IF J=1 THEN 450
320 GOSUB 3120:PRINT"You have arrived at "LA$(J)", "LB$(J)" at";:GOSUB 3070
330 IF TN>1 THEN PRINT"just "TN" minutes late.":GOTO 360
340 IF TN<-1 THEN PRINT"almost "abs(TN)" minutes early.":GOTO 360
350 PRINT" right on time!"
360 IF TB>TD(J)-2 THEN T=TB+4 ELSE T=TD(J)
370 IF J=24 THEN 3190 ELSE IF ME(J)<4 THEN 400
380 PRINT"Asleep in your compartment, you barely notice that the"
390 PRINT"departure was right on time at";:GOSUB 3070:GOSUB 2870:GOTO 490
400 IF J=23 THEN GOSUB 1350
410 PRINT"Departure is at";:GOSUB 3070:PRINT
420 INPUT"Would you like to get off and stretch your legs";A$:GOSUB 2820
...
470 PRINT:SOUND 1,130,15:GOSUB 2870:SOUND 1,130,30:PRINT"All aboard ";
480 GOSUB 2870:PRINT"train is leaving.":GOSUB 2870
490 GOSUB 3000
500 GOSUB 2870:IF J>1 THEN 570
...
570 IF J=23 THEN GOSUB 1500
580 IF ME(J)>0 AND ME(J)<4 THEN ON ME(J)GOSUB 780,730,650
590 GOSUB 870
600 IF HZ(J)>0 THEN ON HZ(J)GOSUB 980,1120
610 GOSUB 1230
620 NEXT J
...
670 WHILE LEN(INKEY$)=0:WEND:CLS:LOCATE 33,3:PRINT"BREAKFAST MENU"
680 FOR I=1 TO 4:X=3*(I-1)+1+INT(3*RND(1))
690 LOCATE(80-LEN(MB$(X)))/2,4+3*I:PRINT MB$(X):NEXT
700 LOCATE 20,19:PRINT MB$(13):GOSUB 2960:RETURN
...
800 WHILE LEN(INKEY$)=0:WEND:CLS:LOCATE 34,1:PRINT"DINNER MENU"
810 FOR I=1 TO 7:X=3*(I-1)+1+INT(3*RND(1))
820 LOCATE(80-LEN(MD$(X)))/2,2+2*I:PRINT MD$(X):NEXT
830 LOCATE 24,18:PRINT MD$(22):LOCATE 30,20:PRINT MD$(23)
840 LOCATE 32,22:PRINT MD$(24):GOSUB 2960:RETURN
870 FOR K=1 TO CN(J)
880 GOSUB 2900:CM=CM+1
890 IF CP(CS(CM))>0 THEN X=CP(CS(CM))ELSE X=3+INT(23*RND(1))
900 PRINT"Standing there is "N$(X)", who tells you:":X=CS(CM)
910 IF LEN(C$(X))<81 THEN PRINT C$(X):GOTO 950
920 CS(RN)=CS(CM):FOR KA=79 TO 1 STEP-1
930 IF MID$(C$(X),KA,1)<>" "THEN NEXT KA
940 PRINT LEFT$(C$(X),KA):PRINT RIGHT$(C$(X),LEN(C$(X))-KA)
950 NEXT K:RETURN
980 X=RND(1):IF X>0.65 THEN RETURN
990 PRINT:PRINT"It is snowing heavily ";
1000 IF X<0.01 THEN 1030
...
1060 PRINT"Express was stuck in snowdrifts for five days.":PRINT:GOSUB 2870
1070 PRINT"But it looks like it is!":GOSUB 2870
...
1120 IF RND(1)>0.04 THEN RETURN
1130 IF HX=1 THEN RETURN ELSE HX=1
1140 PRINT:PRINT"You are rudely awakened from a deep sleep by a loud noise"
1150 PRINT"as the train jerks to a halt.":GOSUB 2900
...
1180 GOSUB 2870:PRINT"The bandits are off the train in a few moments with"
...
1230 IF RND(1)>0.02 THEN RETURN
1240 IF HW=1 THEN RETURN ELSE HW=1
...
1270 PRINT"leaning at a crazy angle. People are screaming."GOSUB 2870
...
1300 PRINT"train has left the track.":GOSUB 2870:PRINT
...
1420 INPUT"and who is the killer";A2:GOSUB 2870
...
1470 PRINT:GOSUB 2870:RETURN
1500 IF A1=A3 OR A1=A4 THEN 1600
...
1530 PRINT"However, one of the other arms dealers has been shot.":GOSUB 2870
...
1600 IF A2=A4 THEN A5=1:RETURN
1610 GOSUB 2900:PRINT"A man is standing outside. He says, 'You made a"
1620 PRINT"mistake. A bad one. You see, I am the machine-gun dealer."
1630 IF A1<>A4 THEN 1660
...
1660 PRINT:GOSUB 2870:PRINT"He draws a gun. BANG. You are dead."
1670 PRINT:PRINT"You never know that the train arrived at 12:30, right on"
1680 PRINT"time at Constantinople, Turkey.":GOSUB 2870:GOSUB 2870
...
2540 FOR I=1 TO 13:READ MB$(I):NEXT I
2550 FOR I=1 TO 26:READ MD$(I):NEXT I
2560 RETURN
...
2840 INPUT"Please enter Y for 'yes' or N for 'no.' Which is it";A$:GOTO 2820
2870 FOR X=1 TO 1000:NEXT X:RETURN
2900 PRINT:PRINT"Your compartment buzzer rings"
2910 PRINT CHR$(7):FOR KA=1 TO 300:NEXT:PRINT CHR$(7)
2920 PRINT"Press any key to open the door."
2930 WHILE LEN(INKEY$)=0:WEND:RETURN
2960 LOCATE 18,24:PRINT"Press any key when you have finished eating";
2970 A3=A3+5*(J+1)-POS(X):WHILE LEN(INKEY$)=0:WEND:CLS:RETURN
3000 PRINT:PRINT"Clackety clackclackety clackclackety clack"
3010 IF RND(1)>0.5 THEN RETURN ELSE FOR KA=6 TO 1 STEP-1
3020 FOR I=1 TO 4:SOUND 1,130,0.5:FOR K=1 TO 180:NEXT:NEXT
3030 IF KA=4 THEN GOSUB 3120
...
3090 RETURN
3120 IF RND(1)>0.5 THEN RETURN
3130 SOUND 1,130,15:FOR K=1 TO 1400:NEXT:SOUND 1,130,3
3140 FOR K=1 TO 200:NEXT:SOUND 1,130,20:RETURN
...
3240 IF A5<>1 THEN 3310 ELSE PRINT CHR$(8)+CHR$(7)+CHR$(7):X=0
3250 PRINT"Whitehall telegraphs congratulations for identifying both"
3260 PRINT"the killer and defector correctly.":GOSUB 2870:GOSUB 2870
3270 FOR I=1 TO 25:FOR K=1 TO 100:NEXT K:LOCATE 30,15:PRINT X$:PRINT CHR$(7)
3280 IF X=0 THEN X$="CONGRATULATIONS !":X=1:GOTO 3300
3290 X$=" ":X=0
3300 NEXT I:PRINT:PRINT:PRINT
3310 INPUT"Would you like to ride again";A$:GOSUB 2820
3320 IF A=0 THEN PRINT"Okay. Good journey!":GOSUB 2870:RUN
3330 PRINT"Okay. So long for now.":GOSUB 2870:CLS:END