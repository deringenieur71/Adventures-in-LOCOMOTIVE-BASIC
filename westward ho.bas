...
110 LOCATE 31,10:PRINT"Westward Ho! 1847":PRINT:PRINT
120 PRINT TAB(28)"(c) David H. Ahl, 1986":PRINT:PRINT TAB(15)"converted to CPC 2017 by SRS *** www.cpcwiki.eu ***":LOCATE 21,23
150 DIM DA$(20),EP(20),MP(15),PL$(15)
160 GOSUB 3440:GOSUB 3530:GOSUB 3710
...
210 GOSUB 700
220 GOSUB 930
230 PRINT:PRINT" Your trip is about to begin":PRINT:GOSUB 3840
260 IF M>2039 THEN 3200
270 J=J+1
...
1040 IF X=3 THEN RETURN ELSE ON X GOSUB 1110,1210
...
1080 IF X=2 THEN RETURN ELSE GOSUB 1210:RETURN
...
1150 INPUT"Medicine and supplies";P4:P=P1+P2+P3+P4:P1=0.67*P1:P2=33*P2
1160 PRINT"The storekeeper tallies up your bill. It comes to $"P
1170 IF T>=P THEN T=T-P:F=F+P1:B=B+P2:C=C+0.67*P3:R=R+0.67*P4:RETURN
1180 PRINT"Uh, oh. That's more than you have. Better start over.":GOTO 1130
1210 KQ=0:IF B>39 THEN 1230
1220 PRINT"Tough luck. You don't have enough ammo to hunt.":KQ=1:RETURN
1230 M=M-45:GOSUB 3880:IF BR<=1 THEN 1270
1240 IF 100*RND(1)<13*BR THEN 1290
1250 PRINT"Nice shotright on targetgood eatin' tonight!"
1260 F=F+24-2*BR:B=B-10-3*BR:RETURN
1270 PRINT"Right between the eyesyou got a big one!":F=F+26+3*RND(1)
1280 PRINT"Full bellies tonight!":B=B-10-4*RND(1):RETURN
1290 PRINT"You missed completelyand your dinner got away.":RETURN
1320 IF F<5 THEN 3000
...
1350 F=F-4-2.5*E:IF F>0 THEN RETURN
...
1410 X$="":GH=0:IF RND(1)>0.2 THEN X$="don't ":GH=1
...
1460 IF RND(1)<0.2 THEN GH=1-GH
1470 IF GH=1 THEN 1680
1480 ON GT GOTO 1510,1540,1610,1650
1510 M=M+20:R=R-7:B=B-150:A=A-20:GOTO 1730
1540 GOSUB 3880:B=B-BR*40-80
...
1760 IF B>=0 THEN RETURN ELSE PRINT:GOSUB 3750:PRINT"Oh, my gosh!";
...
1860 ON I-8 GOTO 2290,2320,2350,2410,2440,2530,2570,2610
...
1940 PRINT"Bad luckyour daughter breaks her arm. You must stop and"
...
2130 PRINT"Cold weatherBrrrrrrr!You ";
2140 IF C<11+2*RND(1)THEN PRINT"don't ";:C1=1
2150 PRINT"have enough clothing to keep warm."
2160 IF C1=0 THEN RETURN ELSE GOSUB 2890:RETURN
2180 PRINT"Bandits attacking!":GOSUB 3880
...
2220 IF BR<=1 THEN 2260
2230 PRINT"You get shot in the leg  ";:GOSUB 3750:KH=1
2240 PRINT"and they grab one of your oxen.":A=A-10:R=R-2
2250 PRINT"Better have a doc look at your legand soon!":RETURN
...
2440 PRINT"You're sound asleep and you hear a noiseget up to investigate."
2450 GOSUB 3750:PRINT"It's wild animals! They attack you!":GOSUB 3880
...
2490 PRINT"Nice shooting, pardnerThey didn't get much.":RETURN
...
2570 IF E=1 THEN GOSUB 2890:RETURN
2580 IF E=2 AND RND(1)>0.25 THEN GOSUB 2890:RETURN
2590 IF E=3 AND RND(1)>0.5 THEN GOSUB 2890:RETURN
2610 PRINT"Helpful Indians show you where to find more food."
2620 F=F+7:RETURN
2650 IF M<=975 THEN RETURN
2660 IF 10*RND(1)>9-((M/100-15)^2+72)/((M/100-15)^2+12)THEN 2760
2670 PRINT"You're in rugged mountain country.":IF RND(1)>0.1 THEN 2700
2680 PRINT"You get lost and lose valuable time trying to find the trail."
2690 M=M-60:GOTO 2760
2700 IF RND(1)>0.11 THEN 2730
2710 PRINT"Trail cave in damages your wagon. You lose time and supplies."
2720 M=M-20-30*RND(1):B=B-200:R=R-3:GOTO 2760
2730 PRINT"The going is really slow; oxen are very tired.":M=M-45-50*RND(1)
2760 IF KP=1 THEN 2790
2770 KP=1:IF RND(1)<0.8 THEN 2840
2780 PRINT"You made it safely through the South Pass....no snow!"
2790 IF M<1700 THEN 2810
2800 IF KM=1 THEN 2810
2810 KM=1:IF RND(1)<0.7 THEN 2840 ELSE RETURN
2820 MP=1:RETURN
2840 PRINT"Blizzard in the mountain pass. Going is slow; supplies are lost."
2850 KB=1:M=M-30-40*RND(1):F=F-12:B=B-200:R=R-5
2860 IF C<18+2*RND(1)THEN 2890 ELSE RETURN
...
3100 PRINT:GOSUB 3750:PRINT"You stumble and can't get up...."
3110 PRINT:GOSUB 3750:PRINT"Some travelers find the bodies of you and your"
...
3170 PRINT" You had a few supplies left :":GOSUB 3360:PRINT:GOTO 3310
3200 ML=(2040-MA)/(M-MA):F=F+(1-ML)*(8+5*E):GOSUB 3840
...
3250 PRINT"You have few supplies remaining :":GOSUB 3360
...
3280 GOTO 3310
3310 PRINT:INPUT"Would you like to play again";A$:GOSUB 3780
3320 IF A=0 THEN PRINT"Okay, good luck!":GOSUB 3750:RUN
3330 PRINT"Okay. So long for now.":GOSUB 3750:CLS:END
...
3840 FQ=220:SOUND 1,65000/FQ,3:SOUND 1,65000/(FQ*1.333),3:SOUND 1,65000/(FQ*1.584),3
3850 SOUND 1,65000/(FQ*1.885),5:SOUND 1,65000/(FQ*1.584),2:SOUND 1,65000/(FQ*1.885),10:RETURN
3880 RN=1+INT(4*RND(1))
3890 S1=60*VAL(MID$(zeit$,4,2))+VAL(RIGHT$(zeit$,2))
3900 PRINT"Type "S$(RN);:INPUT X$
3910 IF S$(RN)<>X$AND S$(RN+4)<>X$THEN PRINT"Nope. Try again. ";:GOTO 3900
3920 S2=60*VAL(MID$(zeit$,4,2))+VAL(RIGHT$(zeit$,2))
