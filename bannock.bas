5 DIM B$(12),M$(6),M(7)
6 MODE 1:REM for CPC 03/25 SRS patched 08/25
...
130 M=0:IF Z>1 THEN IF RND>0.5 THEN GOSUB 2740
140 GOSUB 4990
150 GOSUB 1900
160 GOSUB 4990
170 GOTO 40
...
690 INPUT"Attribute (M, S or W)";E$:E$=UPPER$(E$)
700 IF E$="N"THEN 790
710 INPUT"Amount";AM
...
1890 REM *******************************
1900 REM Action
1910 D=4:IF MID$(B$(Z),9,1)="0"OR MID$(B$(Z),9,1)=" "THEN D=1:IF RND>0.8 AND Z>1 THEN GOSUB 2740:GOTO 1910
1920 PRINT"What do you want to do now";
1930 INPUT Z$:Z$=UPPER$(Z$):IF Z$="Q"THEN END
...
2120 INPUT Z$:Z$=UPPER$(Z$):IF Z$<>"M"AND Z$<>"S"AND Z$<>"W"THEN 2120
...
3060 GOSUB 4990
3070 IF RND>0.6 THEN PRINT"It contained a potion to":PRINT"enhance your wisdom":W(7)=W(7)+INT(RND*6)+1:GOSUB 4990:RETURN
...
3150 IF RND>0.5 THEN PRINT"You cannot understand":PRINT"the language...":GOSUB 4990:RETURN
3160 PRINT"It contains a magic spell. Do":PRINT"you wish to read it?"
3170 GOSUB 3380:IF Z$="N"THEN RETURN
3180 IF RND>0.5 THEN PRINT"It was a beneficent spell":GOSUB 4990:M(7)=M(7)+INT(RND*6)+1:RETURN
...
3440 Q=0:IF RND>0.81 AND Z>1 THEN Q=INT(RND*6):MID$(B$(Z),9,1)=MID$(STR$(Q),2,1):GOTO 260
...
4580 INPUT"Please enter your first name";N$:N$=UPPER$(n$)
...
4660 IF T>1 AND T<11 AND L<0.63 THEN B$(T)=B$(T)+RIGHT$(Q$,1)
4670 IF T>10 OR L>=0.63 THEN B$(T)=B$(T)+"0"
4680 NEXT T
4690 L=RND
4700 IF L<0.5 THEN B$(11)="091200000"
4710 IF L>=0.5 THEN B$(10)="061200000"
...
5000 FOR i=1 TO 500:NEXT
5010 PRINT
5020 RETURN                               
6000 FOR i=1 TO 500+xx*500:NEXT:RETURN