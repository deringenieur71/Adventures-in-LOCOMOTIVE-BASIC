...
310 IF DZ=0 AND RND(1)<0.18 THEN GOSUB 3020:'18% chance to hunt for food
...
800 B=A:JL=JL-BA*B:A2=3*B-6:'Camelsnumber, cost, amount they can carry
...
1050 IF FE=3 THEN PFD=PFD+0.4
1060 IF PSKT+PWDT+PFD<3 THEN RETURN
1070 IF RND(1)>0.7 THEN RETURN:'70% chance of delay due to recurring illness
...
1440 IF RND(1)>0.2 THEN PRINT"warn you not to return.":CZ=1:RETURN
...
1580 FR=INT(0.5+10*(F-FE))/10:IF FR>3 THEN 1630
...
1870 IF L>1 THEN IF RND(1)>0.5 THEN L=0:GOTO 1900
...
2130 BL=B:IF BSK<=J THEN BL=B-0.6:BA=BA-1:'If sick camel reduce load, speed
2140 IF F+L<=3*BL THEN RETURN
2150 PRINT"You have too large a load for your camels.":FC=INT(F+L-3*BL+0.9)
...
2430 F=F-0.4:C=C-1:GOSUB 3110:IF L<1 THEN RETURN ELSE L=L-0.5:RETURN
2440 '
...
2490 PWD=0.5:GOSUB 2840:RETURN
2500 '
...
2550 GOSUB 2840:D=D-50:PWD=0.7:RETURN
2560 '
...
2640 PRINT"Your party slows down for you.":PSK=0.7:D=D-125:RETURN
2650 '
...
2720 IF RND(1)>0.8 THEN 2750:'80% chance of surviving attack
...
2860 IF RND(1)>0.5 THEN XA$="balm"ELSE XA$="unguent"
...
2950 PRINT"Your wound is badly infected, ";:IF RND(1)<0.8 THEN 3000
...
3390 X$="CONGRATULATIONS !":LOCATE 12,1:GOSUB 3760
...
3770 PRINT TAB((80-LEN(X$))/2);X$;:RETURN
3780 '
